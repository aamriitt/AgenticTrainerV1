...
import { Filter, Grid3x3, List, Plus, SlidersHorizontal, ArrowUpDown } from "lucide-react";
...
export function RepositoryPage() {
  ...
  const smes = useMemo(() => {
    if (!items) return [];
    return Array.from(new Set(items.map((i) => i.sme)));
  }, [items]);
  ...
  const filtered = useMemo(() => {
    let result = (items ?? []).filter((it) => {
      ...
      return matchesTab && matchesQuery && matchesSme;
    });
    ...
    return result;
  }, [items, tab, query, selectedSme, sortOrder]);
    ...
    <div>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3.5">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button
            variant="secondary"
            size="sm"
            onClick={() => setSortOrder((prev) => (prev === "title" ? "date" : "title"))}
          ...
          <div className="flex overflow-hidden rounded-xl border border-border bg-card">
            <button
              ...
              className={cn("flex h-8 w-8 items-center justify-center transition-colors", view === "list" ? "bg-primary/10 text-primary font-bold" : "text-muted-foreground")}
              title="List View"
            >
            ...
            <button
              ...
              className={cn("flex h-8 w-8 items-center justify-center transition-colors", view === "grid" ? "bg-primary/10 text-primary font-bold" : "text-muted-foreground")}
              title="Grid View"
            >
      ...
      {isLoading ? (
        ...
        <Card className="p-10 text-center rounded-2xl border-dashed">
          ...
          <Button variant="secondary" size="sm" onClick={() => { setQuery(""); setTab("all"); setSelectedSme("all"); }} className="mt-3 rounded-xl">
            Reset Filters
          </Button>
        </Card>
      ) : view === "grid" ? (
        <div className="grid grid-cols-1 gap-3.5 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((item) => (
            <KnowledgeCard key={item.id} item={item} onClick={() => setPreview(item)} />
          ))}
        </div>
      ) : (
          ...
          <table className="w-full min-w-[720px] border-collapse text-sm">
            <thead>
              <tr className="bg-secondary/60 text-left border-b border-border">
                {["Title", "SME Owner", "Uploaded Date", "Tags", "Status", "Vector Embedding"].map((h) => (
                ...
                >
                  ...
                  <td className="px-4 py-3.5">
                    <div className="flex flex-wrap gap-1">
                      {item.tags.map((t) => <Badge key={t} variant="indigo">{t}</Badge>)}
                    </div>
                  </td>
